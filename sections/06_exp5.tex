...
\begin{equation}
    R = \sigma^2_\text{sensor} = 0.0765 \; \text{cm}^2
    \label{eq:R_exp5}
\end{equation}
...
\begin{lstlisting}[language=C,
    ...
    label={lst:main_exp5}]
/* System parameters from Experiments 1 and 4 */
#define MSD_K         29.11f   /* Spring constant (N/m)           */
#define MSD_M         [m_real] /* Mass from scale (kg)            */
#define MSD_C         [C_real] /* Damping from Parameter Estimator*/

/* KF noise parameters */
#define KALMAN_DT     0.010f   /* Sampling time Ts = 10 ms        */
#define KALMAN_Q0     [q1]     /* Position process noise           */
#define KALMAN_Q1     [q2]     /* Velocity process noise           */
#define KALMAN_R      0.0765f  /* Measurement noise (cm^2)        */
...
Kalman2_Init(&kf_msd,
             KALMAN_DT, KALMAN_Q0, KALMAN_Q1, KALMAN_R,
             raw_cm, 0.0f);   /* x0 = first reading, v0 = 0 */
...
while (1) {
    ...
    if (raw_cm > 0.0f) {
        float pos_est = Kalman2_Update(&kf_msd, raw_cm);
        float vel_est = Kalman2_GetVelocity(&kf_msd);

        uint32_t t_ms = HAL_GetTick();
        printf("%lu,%.4f,%.4f,%.4f\r\n",
               t_ms, raw_cm, pos_est, vel_est);
        fflush(stdout);
    }
    HAL_Delay(10);   /* Must match KALMAN_DT = 0.010f */
}
\end{lstlisting}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        Measurement Noise       & $R$    & cm$^2$      & $0.0765$  \\
...
\begin{table}[htbp]
    ...
    \begin{tabular}{cccc}
        ...
        \hline
        2~cm & $\text{diag}(q_1, q_2)$ & $0.0765$ & [เชิงคุณภาพ] \\
        4~cm & $\text{diag}(q_1, q_2)$ & $0.0765$ & [เชิงคุณภาพ] \\
        6~cm & $\text{diag}(q_1, q_2)$ & $0.0765$ & [เชิงคุณภาพ] \\
        \hline
    \end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{$R$ (Measurement Noise):}
          ...
          ค่า $R = 0.0765$~cm$^2$ บอกว่า Filter ควรเชื่อ Measurement
