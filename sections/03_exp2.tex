...
\begin{itemize}
    \item เพื่อพัฒนา Library \texttt{hcsr04.c}/\texttt{hcsr04.h} สำหรับอ่านค่า
          ...
          ให้มีความถูกต้องและรองรับคณิตศาสตร์แบบจุดลอยตัว
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.70\textwidth]{images/Ultrasonic-timing-control-The-speed-of-sound-is-roughly-340-meters-per-second-or-around.jpg}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{images/ultra3.png}
...
และ $P(t)$ คือ Error Covariance ที่วิวัฒน์ตามสมการ Riccati
...
\begin{equation}
    F = e^{A_c T_s} \approx I + A_c T_s = 1 + (0)(T_s) = 1
    \label{eq:discretize_F}
\end{equation}

ผลลัพธ์ที่ได้คือ Discrete State-Space Model สำหรับ Scalar KF

\begin{align}
    x_k     &= F x_{k-1} + B u_{k-1} + w_{k-1} = x_{k-1} + w_{k-1}
    \label{eq:scalar_state} \\
    z_k     &= H x_k + v_k = x_k + v_k
    \label{eq:scalar_obs}
\end{align}

ซึ่งสอดคล้องกับสมมติฐานที่ว่าระยะทางที่แท้จริงไม่เปลี่ยนแปลงระหว่าง
Sampling Period และ Sensor วัดสถานะได้โดยตรงแต่มี Noise
จากสมการ~\eqref{eq:scalar_state}--\eqref{eq:scalar_obs}
สามารถเขียนสมการ Discrete Kalman Filter ทั้ง 5 สมการได้ดังนี้
...
\begin{lstlisting}[language=C, caption={Scalar Kalman Filter — kalman.c},
                   label={lst:scalar_kf}]
/* kalman.h — Scalar Kalman Filter state */
typedef struct {
    float x_est;   /* State estimate (position, cm)          */
    float P;       /* Error covariance                        */
    float Q;       /* Process noise covariance                */
    float R;       /* Measurement noise covariance (cm^2)     */
} KalmanScalar_t;

/* Initialise filter */
void Kalman_Init(KalmanScalar_t *kf,
                 float Q, float R, float x0)
{
    kf->x_est = x0;  /* Initial state = first measurement    */
    kf->P     = 1.0f;/* Initial covariance (large = uncertain)*/
    kf->Q     = Q;
    kf->R     = R;
}

/* Run one Predict-Update cycle, return filtered estimate */
float Kalman_Update(KalmanScalar_t *kf, float z)
{
    /* --- Prediction Step --- */
    /* Eq.(kf_predict_state): x_prior = x_est  (F = 1) */
    float x_prior = kf->x_est;

    /* Eq.(kf_predict_cov): P_prior = P + Q */
    float P_prior = kf->P + kf->Q;

    /* --- Update Step --- */
    /* Eq.(kf_gain): K = P_prior / (P_prior + R) */
    float K = P_prior / (P_prior + kf->R);

    /* Eq.(kf_update_state): x_est = x_prior + K*(z - x_prior) */
    kf->x_est = x_prior + K * (z - x_prior);

    /* Eq.(kf_update_cov): P = (1 - K) * P_prior */
    kf->P = (1.0f - K) * P_prior;
...
float HCSR04_Read(void) {
    ...
    /* Step 1: Send 10 us trigger pulse */
    ...
    /* Step 2: Wait for Echo HIGH (rising edge) */
    __HAL_TIM_SET_COUNTER(hcsr04_htim, 0);
    timeout_start = HAL_GetTick();
    while (HAL_GPIO_ReadPin(HCSR04_ECHO_PORT, HCSR04_ECHO_PIN)
           == GPIO_PIN_RESET) {
    ...
    /* Step 3: Wait for Echo LOW (falling edge) */
    timeout_start = HAL_GetTick();
    while (HAL_GPIO_ReadPin(HCSR04_ECHO_PORT, HCSR04_ECHO_PIN)
           == GPIO_PIN_SET) {
    ...
    /* Step 4: Convert pulse width to distance (cm)
     * s = T_us / 58.0  (v_sound = 343.4 m/s at 20 C) */
    return (float)(value2 - value1) / 58.0f;
}
\end{lstlisting}
...
/* Main loop: read sensor and send raw distance over Serial */
while (1) {
    float raw_cm = HCSR04_Read();

    if (raw_cm > 0.0f) {   /* skip timeout readings (-1.0f) */
        uint32_t t_ms = HAL_GetTick();
        printf("%lu,%.4f\r\n", t_ms, raw_cm);
        fflush(stdout);
    }

    HAL_Delay(60);   /* 60 ms between readings (min interval for HC-SR04) */
}
\end{lstlisting}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{cccc}
        ...
        \textbf{ระยะ (cm)}
            ...
            & \textbf{S.D. ของ $s^2$ ข้าม Trials (cm$^2$)} \\
        \hline
        10 & 9.297  & 0.0050  & {[ข้อมูลจริง]} \\
        20 & 18.847 & 0.0337  & {[ข้อมูลจริง]} \\
        30 & 28.954 & 0.0734  & {[ข้อมูลจริง]} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{images/HC-SR04 Sensor Measurements Analysis.png}
...
\begin{itemize}
    \item สามารถพัฒนา Library \texttt{hcsr04.c}/\texttt{hcsr04.h}
          ...
          โดยใช้ TIM1 เป็น Free-Running Counter ความละเอียด $1\,\mu$s
...
\textit{International Journal of Innovative Technology and Exploring Engineering
(IJITEE)}, \textit{2}(1), 15--19.

\bigskip
\noindent
STMicroelectronics. (2023).
\textit{STM32G4 Series reference manual (RM0440)}.
STMicroelectronics.

\bigskip
\noindent
Elecfreaks. (2012).
\textit{HC-SR04 ultrasonic ranging module: User manual}.
Elecfreaks.